...
class Smartwave_Mango_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    {
        $this->_texturePath = 'wysiwyg/mango/texture/default/';
    }

    public function getCfgGroup($group, $storeId = NULL)
    {
        if ($storeId)
            return Mage::getStoreConfig('mango/' . $group, $storeId);
        else
            return Mage::getStoreConfig('mango/' . $group);
    }
    
    public function getCfgSectionDesign($storeId = NULL)
    {
        if ($storeId)
            return Mage::getStoreConfig('mango_design', $storeId);
        else
            return Mage::getStoreConfig('mango_design');
    }

    public function getCfgSectionSettings($storeId = NULL)
    {
        if ($storeId)
            return Mage::getStoreConfig('mango_settings', $storeId);
        else
            return Mage::getStoreConfig('mango_settings');
    }
    ...
    {
        return Mage::getStoreConfig('mango_settings/' . $optionString);
    }
    ...
    public function isEnabledonConfig($id){
        $store = Mage::app()->getStore();
        $code  = $store->getCode();
        if(Mage::getStoreConfig("mango_settings/product_view_custom_tab/custom_tab",$code))
            return true;
        if(Mage::getStoreConfig("mango_settings/product_view_custom_tab/".$id,$code))
            return true;
        return false;
    }
    public function isEnabledfromCategory(){
        $store = Mage::app()->getStore();
        $code  = $store->getCode();
        if(Mage::getStoreConfig("mango_settings/product_view_custom_tab/from_category",$code))
            return true;
        return false;
    }
    ...
    public function isEnabledTab($type, $id, $product_id){
        ...
        $config_id = Mage::getStoreConfig("mango_settings/product_view_custom_tab/".$this->getTabIdField($type,$id),$code);
        if(!$config_id)
            return false;
